...
CommandLineParser::CommandLineParser(QObject* _parent) : QObject(_parent), m_parser(), m_helpOption(m_parser.addHelpOption()),
  ...
  m_p2pBindPortOption("p2p-bind-port", tr("Port for p2p network protocol"), tr("port"), QString::number(config::get().net.p2p_port)),
  ...
  m_dataDirOption("data-dir", tr("Specify data directory"), tr("directory"), QString::fromStdString(os::appdata::path())),
  m_minimized("minimized", tr("Run application in minimized mode")) {
  m_parser.setApplicationDescription(tr("VigCoin wallet"));
